...
def average_pixel(src, i, j):
    img = cv.imread(src, 0)
    value = int((img[i][j] + img[i+1][j] + img[i][j+1] + img[i+1][j+1]) / 4)
    #print(value)
    return value

def shrink(src):
    '''
    following code has runtime error

    res = cv.imread(src, 0)
    row = np.shape(res)[0]
    col = np.shape(res)[1]
    img_out = np.zeros((int(row / 2), int(col / 2)), dtype=np.int32)
    for i in range(0, int(row / 2), 1):
        for j in range(0, int(col / 2), 1):
            img_out[i][j] = res[i * 2][j * 2]  # average_pixel(src, i*2, j*2)
